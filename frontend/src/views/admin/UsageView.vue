...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.type'), t('admin.usage.schedule'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.inbound_endpoint || '', log.upstream_endpoint || '', getRequestTypeLabel(log), log.schedule_trace?.layer || '',
...
const allColumns = computed(() => [
  ...
  { key: 'schedule_trace', label: t('admin.usage.schedule'), sortable: false },
