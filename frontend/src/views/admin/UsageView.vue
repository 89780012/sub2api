...
import { formatSelectionCompareExport, formatSelectionReasonExport } from '@/utils/usageSelectionReason'
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.selectionReason'), t('usage.selectionCompare'),
      t('usage.type'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.inbound_endpoint || '', log.upstream_endpoint || '', formatSelectionReasonExport(log.selection_reason, t), formatSelectionCompareExport(log.selection_reason, t), getRequestTypeLabel(log),
...
const allColumns = computed(() => [
  ...
  { key: 'selection_compare', label: t('usage.selectionCompare'), sortable: false },
